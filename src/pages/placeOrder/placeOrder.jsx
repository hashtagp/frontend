...
const PlaceOrder = () => {
  ...
  const salesTax = Math.round(getTotalCartValue() * 0.18); // 18% GST
  ...
  const orderTotal = getTotalCartValue() + shippingCharge + salesTax - (couponApplied ? couponDiscount : 0);
  ...
  const placeOrder = async (event) => {
    ...
    try {
      ...
      const totalAmount = getTotalCartValue() + shippingCharge + salesTax - (couponApplied ? couponDiscount : 0);

      let orderData = {
        address: data,
        items: orderItems,
        amount: totalAmount,
        itemTotal: getTotalCartValue(),
        salesTax: salesTax,
